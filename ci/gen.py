...
for i in sorted(vms):
    ...
    outs += 'sudo hostname %(name)s\n'\
            ...
            '   sudo rlite-ctl ipcp-create n.${i}.IPCP:%(id)s normal n.${i}.DIF\n'\
            '   sudo rlite-ctl ipcp-config n.${i}.IPCP:%(id)s address %(id)d\n'\
            'done\n'\
            '\n' % d
    ...
    for port in vm['ports']:
        ...
        outs += 'PORT=$(mac2ifname %(mac)s)\n'\
                ...
                'sudo rlite-ctl ipcp-create e.%(brid)s.IPCP:%(idx)s shim-%(shimtype)s e.%(brid)s.DIF\n' % vars_dict
        if args.type == 'eth':
                outs += 'sudo rlite-ctl ipcp-config e.%(brid)s.IPCP:%(idx)s netdev $PORT\n' % vars_dict
        elif args.type == 'tcp4':
        ...
        outs += 'sudo rlite-ctl ipcp-register e.%(brid)s.DIF n.1.IPCP:%(id)s\n'\
                '\n' % vars_dict

    outs += 'for i in $(seq 2 %(levels)s); do\n'\
            '   sudo rlite-ctl ipcp-register n.$(($i-1)).DIF n.$i.IPCP:%(id)s\n'\
            'done\n'\
            'true\n'\
            'ENDSSH\n'\
            '   DONE=$?\n'\
