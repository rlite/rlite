                           ...
                           const std::string &remote_node)
{
    ...
    freq->set_dst_port(0);
    freq->set_src_ipcp(rib->myname);
    freq->set_dst_ipcp(remote_node);
    ...
    ret = rib->send_to_dst_node(std::move(m), remote_node, freq.get());
    if (ret) {
        return ret;
    }
...
LocalFlowAllocator::flows_handler_create_r(const CDAPMessage *rm)
{
    ...
    remote_addr = rib->lookup_node_address(freq->dst_ipcp());
                                         ...
                                         const MsgSrcInfo &src)
{
    ...
    remote_addr = rib->lookup_node_address(freq->src_ipcp());
    if (freq->connections_size() < 1 || remote_addr != src.addr) {
...
LocalFlowAllocator::fa_resp(struct rl_kmsg_fa_resp *resp)
{
    ...
    ret = rib->send_to_dst_node(std::move(m), freq->src_ipcp(), freq);
...
LocalFlowAllocator::flow_deallocated(struct rl_kmsg_flow_deallocated *req)
{
    ...
    if (freq->flags & RL_FLOWREQ_INITIATOR) {
        ...
        remote_node = freq->dst_ipcp();
    } else {
        remote_obj_name << freq->src_port();
        remote_node = freq->src_ipcp();
    }
                                         ...
                                         const MsgSrcInfo &src)
{
    ...
    expected_src_addr = rib->lookup_node_address(
        (freq->flags & RL_FLOWREQ_INITIATOR) ? freq->dst_ipcp()
                                             : freq->src_ipcp());
