...
uipcp_rib::fa_req(struct rl_kmsg_fa_req *req)
{
    ...
    ret = dft.lookup_entry(dest_appl, remote_addr);
    if (ret) {
...
uipcp_rib::flows_handler_create(const CDAPMessage *rm)
{
    ...
    ret = dft.lookup_entry(freq.dst_app, dft_next_hop);
    if (ret) {
    ...
    uipcp_issue_fa_req_arrived(uipcp, freq.uid, freq.src_port,
...
uipcp_rib::flow_deallocated(struct rl_kmsg_flow_deallocated *req)
{
    ...
    obj_name << obj_name::flows << "/" << myaddr
                << "-" << req->local_port_id;
...
uipcp_rib::flows_handler_delete(const CDAPMessage *rm)
{
    ...
    ret = uipcp_issue_flow_dealloc(uipcp, local_port, f->second.uid);
